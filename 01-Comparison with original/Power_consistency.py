...
# ORIGINAL vs PIML — Power Consistency
...
plt.plot(orig_p["timestamp"],
         orig_p["voltage"] * orig_p["current"],
         label="Original V×I")

plt.plot(piml_p["timestamp"],
         piml_p["total_power"],
         label="PIML Total Power")

plt.xlabel("Time")
plt.ylabel("Power (W)")
plt.title("Power Consistency: Original vs PIML")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("power_consistency_original_vs_piml.png", dpi=300)
plt.show()

# ORIGINAL vs ML — Power Consistency
...
plt.figure(figsize=(14,5))
plt.plot(orig_m["timestamp"],
         orig_m["voltage"] * orig_m["current"],
         label="Original V×I")

plt.plot(ml_m["timestamp"],
         ml_m["total_power"],
         label="ML Total Power")

plt.xlabel("Time")
plt.ylabel("Power (W)")
plt.title("Power Consistency: Original vs ML")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("power_consistency_original_vs_ml.png", dpi=300)
plt.show()
