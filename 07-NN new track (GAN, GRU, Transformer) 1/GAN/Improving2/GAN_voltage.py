...
def generator_loss(fake_pred, fake_output, real_y):
    adv_loss = keras.losses.binary_crossentropy(tf.ones_like(fake_pred), fake_pred)
    reg_loss = mse_loss(real_y, fake_output)
    return tf.reduce_mean(adv_loss) + 10 * reg_loss  
...
for epoch in range(EPOCHS):
    ...
    X_batch = X_train_seq[idx]
    real_y = y_train_seq[idx].reshape(-1,1)
    
    # Generate fake
    fake_y = generator.predict(X_batch, verbose=0)
    ...
    with tf.GradientTape() as tape:
        fake_output = generator(X_batch, training=True)
        fake_pred = discriminator(fake_output, training=False)
        
        g_loss = generator_loss(fake_pred, fake_output, real_y)
    
    grads = tape.gradient(g_loss, generator.trainable_variables)
    generator.optimizer.apply_gradients(zip(grads, generator.trainable_variables))
    
    if epoch % 5 == 0:
        print(f"Epoch {epoch} | D Loss: {(d_loss_real + d_loss_fake):.4f} | G Loss: {g_loss.numpy():.4f}")
...
results_df.to_csv("voltage_gan_predictions.csv", index=False)